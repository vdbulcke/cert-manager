...
func TestCertParserWrongFormat(t *testing.T) {
	pem := `
...
-----END CERTIFICATE REQUEST-----`
	...
	_, err := ParseX509FromPEM(pem)
	if err == nil {
		t.Logf("Expected error wrong Format %s", err.Error())
		t.FailNow()
	}
...
func TestCertParserX509Fail(t *testing.T) {
	...
	_, err := ParseX509FromPEM(pem)
	if err == nil {
		t.Logf("This PEM string String Tabs should fails")
		t.FailNow()
	}
...
// TBj0/VLZjmmx6BEP3ojY+x1J96relc8geMJgEtslQIxq/H5COEBkEveegeGTLg==
// -----END CERTIFICATE-----
// openssl x509  -text -noout  -sha256 -fingerprint
// Certificate:
//     Data:
//         Version: 3 (0x2)
//         Serial Number:
...
func TestCertParserX509(t *testing.T) {
	pem := `
-----BEGIN CERTIFICATE-----
MIIDujCCAqKgAwIBAgILBAAAAAABD4Ym5g0wDQYJKoZIhvcNAQEFBQAwTDEgMB4G
A1UECxMXR2xvYmFsU2lnbiBSb290IENBIC0gUjIxEzARBgNVBAoTCkdsb2JhbFNp
...
-----END CERTIFICATE-----`
	...
	_, err := ParseX509FromPEM(pem)
	if err != nil {
		t.Logf("Error parsing PEM %s", err.Error())
		t.FailNow()
	}
}

func TestCertParserX509SHA256(t *testing.T) {
...
-----END CERTIFICATE-----`
	...
	cert, err := ParseX509FromPEM(pem)
	if err != nil {
		t.Logf("Error parsing PEM %s", err.Error())
		t.FailNow()
	}

	fingerprint := GetSHA256FingerprintFromX509Cert(cert)

	if fingerprint != "ca42dd41745fd0b81eb902362cf9d8bf719da1bd1b1efc946f5b4c99f42c1b9e" {
		t.Logf("Expecting 'ca42dd41745fd0b81eb902362cf9d8bf719da1bd1b1efc946f5b4c99f42c1b9e', but got '%s'", fingerprint)
		t.FailNow()
	}
}

func TestCertParserX509Subject(t *testing.T) {
...
-----END CERTIFICATE-----`
	...
	subject := GetSubjectFromX509Cert(cert)
	expect := strings.ReplaceAll("CN = GlobalSign,OU = GlobalSign Root CA - R2,O = GlobalSign", " = ", "=")
	if subject != expect {
		t.Logf("Expecting '%s', but got '%s'", expect, subject)
		t.FailNow()
	}
}

func TestCertParserX509SerialNumber(t *testing.T) {
...
-----END CERTIFICATE-----`
	...
	serialNumber := GetSerialNumberFromX509Cert(cert)
	expect := "0400000000010f8626e60d"
	if serialNumber != expect {
		t.Logf("Expecting '%s', but got '%s'", expect, serialNumber)
		t.FailNow()
	}
}

func TestCertParserX509AKI(t *testing.T) {
...
-----END CERTIFICATE-----`
	...
	aki := GetAKIFromX509Cert(cert)
	expect := "9B:E2:07:57:67:1C:1E:C0:6A:06:DE:59:B4:9A:2D:DF:DC:19:86:2E"
	if aki != expect {
		t.Logf("Expecting '%s', but got '%s'", expect, aki)
		t.FailNow()
	}
}

func TestCertParserX509SKI(t *testing.T) {
...
-----END CERTIFICATE-----`
	...
	ski := GetSKIFromX509Cert(cert)
	expect := "9B:E2:07:57:67:1C:1E:C0:6A:06:DE:59:B4:9A:2D:DF:DC:19:86:2E"
	if ski != expect {
		t.Logf("Expecting '%s', but got '%s'", expect, ski)
		t.FailNow()
	}
}
